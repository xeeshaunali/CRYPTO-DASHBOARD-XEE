# app.py – SUPER BOT v4 (Merged v3 + v4) – ENHANCED HISTORICAL OHLCV FETCHING + volume_diff
...
# ========= NEW: PATTERN-BASED PREDICTION SYSTEM =========
# Additional Tables for Patterns and Weights
def init_pattern_tables():
    ...
    try:
        ...
        for tf in timeframes:
            print(f"Creating/verifying tables for timeframe: {tf}")
            
            # Patterns table - larger column + PREFIX index
            cur.execute(f"""
            CREATE TABLE IF NOT EXISTS patterns_{tf} (
                pattern_string VARCHAR(1024) NOT NULL,
                outcomes JSON,
                occurrences INT DEFAULT 1,
                PRIMARY KEY (pattern_string(250))  -- prefix index: only first 250 chars
            ) ENGINE=InnoDB
              DEFAULT CHARSET=utf8mb4
              ROW_FORMAT=DYNAMIC
            """)
            print(f"  → patterns_{tf} OK (with 250-char prefix key)")
            
            # Weights tables - same prefix approach
            for pred_type in ['close', 'high', 'low']:
                table_name = f"weights_{pred_type}_{tf}"
                cur.execute(f"""
                CREATE TABLE IF NOT EXISTS {table_name} (
                    pattern_string VARCHAR(1024) NOT NULL,
                    weight DOUBLE DEFAULT 1.0,
                    PRIMARY KEY (pattern_string(250))
                ) ENGINE=InnoDB
                  DEFAULT CHARSET=utf8mb4
                  ROW_FORMAT=DYNAMIC
                """)
                print(f"  → {table_name} OK (with 250-char prefix key)")
        
        # Confidence table (unchanged)
        cur.execute("""
        CREATE TABLE IF NOT EXISTS confidence_thresholds (
            timeframe VARCHAR(20) PRIMARY KEY,
            perfect_pct DOUBLE DEFAULT 0.0
        ) ENGINE=InnoDB
          DEFAULT CHARSET=utf8mb4
          ROW_FORMAT=DYNAMIC
        """)
        print("→ confidence_thresholds OK")
        
        conn.commit()
        print("\nAll pattern-related tables created/verified successfully!")
        
    except mysql.connector.Error as e:
        print("\nMySQL Error during table creation:")
...
# Helper: Compute triplet (%close, %high, %low) for a candle vs previous close
...
# Helper: Pattern string from list of triplets
def pattern_to_string(pattern):
    return '|'.join([f"{c:.2f}|{h:.2f}|{l:.2f}" for c, h, l in pattern])

# Similarity metric (Euclidean distance for triplets)
...
def train_bot():
    ...
    if look_back < 10 or look_back > 200:
        return jsonify({"success": False, "error": "Look back must be 10-200"}), 400
    ...
    for tf in timeframes:
        ...
        # Rename to capitalized columns (matches compute_triplet)
        ...
        # Build and store patterns (MySQL 5.7 compatible - append in Python)
        conn = get_conn()
        cur = conn.cursor()
        
        for i in range(len(triplets) - look_back):
            ...
            cur.execute(f"SELECT outcomes FROM patterns_{tf} WHERE pattern_string = %s", (pattern_str,))
            row = cur.fetchone()
            ...
            if row:
                ...
                cur.execute(f"""
                    ...
                    WHERE pattern_string = %s
                """, (updated_json, pattern_str))
            else:
                # Insert new
                cur.execute(f"""
                    INSERT INTO patterns_{tf} (pattern_string, outcomes, occurrences)
                    VALUES (%s, %s, 1)
                """, (pattern_str, json.dumps([new_outcome])))
        
        conn.commit()
        cur.close()
        conn.close()
        
        # Initialize weights (default 1.0) for all existing patterns
        conn = get_conn()
        cur = conn.cursor()
        for pred_type in ['close', 'high', 'low']:
            cur.execute(f"""
                INSERT IGNORE INTO weights_{pred_type}_{tf} (pattern_string)
                SELECT pattern_string FROM patterns_{tf}
            """)
        conn.commit()
        cur.close()
        conn.close()
    ...
    return jsonify({"success": True, "message": f"Trained {trained}/{len(timeframes)} timeframes for {symbol}"})
...
def predict_candle():
    ...
    similarity_threshold = 0.05  # configurable later
    ...
    if len(rows) < look_back + 1:
        return jsonify({"success": False, "error": "Not enough recent data"})
    
    df_recent = pd.DataFrame(rows)
    df_recent['close'] = df_recent['close'].astype(float)  # ensure numeric
    # Note: we don't have time_utc here, but we don't need it for triplets
    
    # Compute current pattern (last look_back triplets)
    current_triplets = []
    for i in range(1, len(df_recent)):
        prev_close = df_recent.iloc[i-1]['close']
        triplet = compute_triplet(df_recent.iloc[i], prev_close)
        current_triplets.append(triplet)
    current_pattern = current_triplets[-look_back:]
    current_pattern_str = pattern_to_string(current_pattern)
    
    # Fetch historical patterns
    conn = get_conn()
    cur = conn.cursor(dictionary=True)
    cur.execute(f"""
        SELECT pattern_string, outcomes
    ...
    for hist in hist_patterns:
        # Parse pattern string back to list of triplets
        pattern_parts = [t.split('|') for t in hist['pattern_string'].split('|') if t]
        parsed_pattern = [tuple(map(float, p)) for p in pattern_parts]
        
        dist = pattern_distance(current_pattern, parsed_pattern)
        if dist <= similarity_threshold or hist['pattern_string'] == current_pattern_str:
            # Get weights
            conn = get_conn()
            cur = conn.cursor(dictionary=True)
            weights = {}
            for pred_type in ['close', 'high', 'low']:
                cur.execute(f"SELECT weight FROM weights_{pred_type}_{timeframe} WHERE pattern_string = %s", (hist['pattern_string'],))
                row = cur.fetchone()
            ...
            matches.append({
                'outcomes': json.loads(hist['outcomes']),
                'weights': weights,
                'dist': dist
            })
    
    if not matches:
        return jsonify({"success": False, "error": "No matching patterns found"})
    
    # Weighted average next moves
    total_weight = sum(min(w['weights']['close'], w['weights']['high'], w['weights']['low']) for w in matches)
    if total_weight == 0:
        total_weight = 1  # avoid div0
    
    pct_close = sum(sum(o[0] for o in m['outcomes']) / len(m['outcomes']) * m['weights']['close'] for m in matches) / total_weight
    pct_high = sum(sum(o[1] for o in m['outcomes']) / len(m['outcomes']) * m['weights']['high'] for m in matches) / total_weight
    pct_low = sum(sum(o[2] for o in m['outcomes']) / len(m['outcomes']) * m['weights']['low'] for m in matches) / total_weight
    ...
    # Confidence: % perfect matches historically (simulate from outcomes)
    perfect_count = 0
    total_outcomes = 0
    for m in matches:
        for o in m['outcomes']:
            if close_low <= o[0] <= close_high and high_low <= o[1] <= high_high and low_low <= o[2] <= low_high:
                perfect_count += 1
            total_outcomes += 1
...
if __name__ == "__main__":
    init_tables()
    init_pattern_tables()  # Ensure pattern tables exist
    start_websockets()
