...
def start_websockets():
    ...
    def ws_thread(url, on_open=None, on_message=None, exchange=''):
        while True:
            try:
                print(f"Starting WebSocket for {exchange}...")
                ws = websocket.WebSocketApp(url, 
                                           on_open=on_open, 
                                           on_message=on_message,
                                           on_error=lambda ws, error: print(f"{exchange} WebSocket error: {error}"),
                                           on_close=lambda ws, close_status_code, close_msg: print(f"{exchange} WebSocket closed: {close_msg}"))
                if exchange == 'binance':
                    global binance_ws
                    binance_ws = ws
                elif exchange == 'bybit':
    ...
    def gateio_open(ws):
        ...
        # Subscribe to order book (20 levels, 100ms updates)
        orderbook_sub = {
            "time": timestamp,
        ...
        # Handle trade updates
            ...
            for trade in trades:
                if isinstance(trade, dict):
                    ...
                    # Make sure we convert to float first
                    try:
...
def set_symbol():
    try:
        ...
        if new_symbol != real_time_data["current_symbol"]:
            ...
            # Restart WebSockets with new symbol
            start_websockets()
...
def debug_exchanges():
    ...
    status = {
        ...
        "last_update_time": datetime.fromtimestamp(real_time_data["last_update"]).strftime("%Y-%m-%d %H:%M:%S"),
        "exchanges": {}
    }
...
if __name__ == "__main__":
    ...
    print("🔍 Initial WebSocket status:")
    for exchange in ["binance", "bybit", "okx", "gateio"]:
        book = real_time_data["order_book"][exchange]
        status = "✓ Connected" if book["bids"] or book["asks"] else "✗ No data"
        print(f"   {exchange}: {status}")
    
    print("\n✅ Server ready! Use /debug_exchanges endpoint to check WebSocket status.")
