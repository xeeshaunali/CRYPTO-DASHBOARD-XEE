...
def train_bot():
    ...
    for tf in timeframes:
        ...
        print(f"Trained {tf}: {patterns_added} new, {patterns_updated} updated patterns")
        trained += 1
...
def predict_candle():
    ...
    similarity_threshold = float(payload.get("similarity_threshold", 0.05))
    ...
    if len(rows) < look_back + 1:
        return jsonify({"success": False, "error": f"Not enough recent data: {len(rows)} rows, need {look_back + 1}"})
    ...
    # Fetch historical patterns (with limit for performance)
    conn = get_conn()
    cur = conn.cursor(dictionary=True)
    cur.execute(f"""
        ...
        LIMIT %s
    """, (max_matches * 5,))  # Fetch more initially to find best matches
    hist_patterns = cur.fetchall()
    cur.close()
    conn.close()
    
    if not hist_patterns:
        return jsonify({"success": False, "error": "No historical patterns found for this timeframe"})
    ...
    for hist in hist_patterns:
        ...
        if len(parsed_pattern) != len(current_pattern):
            continue  # Skip if pattern length doesn't match
    ...
    if not matches:
        return jsonify({"success": False, "error": "No matching patterns found (try adjusting similarity_threshold)"})
    
    # Weighted average next moves (weight by both distance and pattern weight)
    total_weight = 0
    weighted_close = 0
    weighted_high = 0
    weighted_low = 0
